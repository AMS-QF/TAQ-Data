...
def get_trades(symbols, start_date, end_date, row_limit):
    ...
    try:
        ...
        for symbol in symbols:
            ...
            command = f'source /home/{server_user}/anaconda3/etc/profile.d/conda.sh && conda activate query_user && cd TAQNYSE-Clickhouse && cd server_helpers && python3 trade_server_helpers.py "{server_user}" "testpassword321" "{symbol}" "{start_date}" "{end_date}" "{row_limit}"'
            ...
            # and save it to the current directory in your local machine
            scp.get('TAQNYSE-Clickhouse/trade_results.csv', f'{symbol}_{start_date.replace("-", "")}-{end_date.replace("-", "")}.csv')
