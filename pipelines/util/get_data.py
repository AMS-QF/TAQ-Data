...
def get_trades(symbols, start_date, end_date, row_limit):
    ...
    try:
        ...
        for symbol in symbols:
            ...
            save_path = project_root / "data" / f'trades_{symbol}_{start_date.replace("-", "")}-{end_date.replace("-", "")}.csv'
...
def get_quotes(symbols, start_date, end_date, row_limit):
    ...
    try:
        ...
        for symbol in symbols:
            ...
            script_path = Path(__file__).resolve().parent
            project_root = script_path.parent
            save_path = project_root / "data" / f'quotes_{symbol}_{start_date.replace("-", "")}-{end_date.replace("-", "")}.csv'
            save_path.parent.mkdir(parents=True, exist_ok=True)
