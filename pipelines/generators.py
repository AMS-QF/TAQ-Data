...
def convertParticipantTimestamp(pts, date):
    ...
    pts = pd.to_datetime(pts.astype(str).str.zfill(15), format="%H%M%S%f")
...
def generate_trade_side(trade_prices):
    ...
    """
    trade_direction_dic = {"uptick": 1, "zero-uptick": 1, "downtick": -1, "zero-downtick": -1, "NaN": np.nan}
...
def generate_transaction_return(df, span, mode="calendar"):
    ...
    if mode == "calendar":
        avgPrice = df[df["Is_Quote"] == False]["Participant_Timestamp_f"].apply(
            lambda t: df[df["Participant_Timestamp_f"].between(t, t + span, inclusive="neither")]["Trade_Price"].mean()
        )
    else:
        avgPrice = None
...
def generate_cal_Lambda(df, delta1, delta2):
    ...
    p_max = df["Participant_Timestamp_f"].apply(
        lambda t: max(
            df[df["Participant_Timestamp_f"].between(t - delta2, t - delta1, inclusive="right")]["Trade_Price"],
            default=np.nan,
        )
    )
...
def generate_cal_LobImbalance(df, delta1, delta2):
    ...
    return df["Participant_Timestamp_f"].apply(
        lambda t: df[df["Participant_Timestamp_f"].between(t - delta2, t - delta1, inclusive="right")][
            "Imbalance"
        ].mean()
    )
...
def getLogPricesForAutoCovForLookBackInterval(df_txn, df_lb, T):
    ...
    # print(interval)
    means_df = (
        interval["Participant_Timestamp_f"]
        .apply(
            lambda t: getLogPricesProduct(df_txn, interval, t)
            if math.isnan(getLogPricesProduct(df_txn, interval, t)) == False
            else None
        )
        .mean()
    )
...
def getEffectiveSpread(df, delta1, delta2, mode):
    ...
    for i in df.index:
        ...
        effective_spread_num = (
            ...
            .apply(lambda t: effectiveSpreadUtil(df, lb_interval_txn, t))
            .sum()
        )
        effective_spread_denominator = (
            lb_interval_txn["Participant_Timestamp_f"]
            .apply(
                lambda t: lb_interval_txn[lb_interval_txn["Participant_Timestamp_f"] == t]["Trade_Volume"].iloc[-1]
                * getPriceForTimeStamp(lb_interval_txn, t)
            )
            .sum()
        )
